<?php 
    ...
    // Validate IELTS score range (0-9)
    if ($listening < 0 || $listening > 9 || 
        $speaking < 0 || $speaking > 9 || 
        $reading < 0 || $reading > 9 || 
        $writing < 0 || $writing > 9) {
        $_SESSION['fail'] = 'IELTS scores must be between 0 and 9 for each section.';
        ob_end_clean();
    ...
    try {
        ...
        $stmt->bind_param("idddds" , $ID , $listening , $speaking , $reading , $writing , $type);
        $stmt->execute();

        $_SESSION['success'] = 'IELTS scores updated successfully.';
        ob_end_clean();
        header("Location: profile2.php");
        exit();
    } catch (mysqli_sql_exception $e) {
        $error_msg = $e->getMessage();
        if (strpos($error_msg, 'IELTS scores must be between 0 and 9') !== false) {
            $_SESSION['fail'] = 'IELTS scores must be between 0 and 9 for each section.';
        } else {
            $_SESSION['fail'] = 'Failed to update IELTS scores: ' . $error_msg;
        }
        ob_end_clean();
        header("Location: profile2.php");
        exit();
